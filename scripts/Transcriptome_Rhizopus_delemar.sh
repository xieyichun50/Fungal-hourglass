...
hisat2 -x Rhizopus_delemar.genomic.fa -1 $id2.r1.fastp.fastq -2 $id2.r2.fastp.fastq -S $id2.sam --phred33 --dta-cufflinks --novel-splicesite-outfile $id2.splicesite.txt  -p 38
...
java -Xmx40g -jar picard.jar MarkDuplicates INPUT=${id2}.sorted.bam OUTPUT=${id2}.sorted.redu.bam METRICS_FILE=${id2}.sorted.redu.metrics.txt REMOVE_DUPLICATES=true ASSUME_SORTED=true
samtools index ${id2}.sorted.redu.bam
done

gff=Rhizopus_delemar.genomic.gff
dir=DEG
cat samples_n_reads_decribed.txt | while read id1 id2 SRR;
do
stringtie -e -B -p 38 -G $gff -A ${id2}_gene_count.xls -o ballgown/${id2}_ballgown/$id2.gtf $id2.sorted.redu.bam
done
perl stringte_gene_count_matrix.pl *_gene_count.xls
