cat samples_n_reads_decribed.txt | while read group id SRR ;
do
...
hisat2 -x /store/jelly/yichun/ccin/ncra/genome/Neurospora_crassa.genomic.fa -1 $id.r1.fastp.fastq -2 $id.r2.fastp.fastq -S $id.sam --fr --phred33 --dta-cufflinks --novel-splicesite-outfile $id.splicesite.txt -p 38
...
gff=/store/jelly/yichun/ccin/ncra/genome/Neurospora_crassa.genomic.gff
dir=DEG
